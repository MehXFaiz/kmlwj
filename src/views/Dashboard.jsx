...
export const Dashboard = () => {
  ...
  const stats = useMemo(() => {
    ...
    return {
      ...
      donationPool: Number(summary.donationPool ?? dbStats.donationPool ?? 0),
    ...
    <div className="space-y-6 pb-10">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-3 2xl:grid-cols-6 gap-3 sm:gap-4">
        {[
          ...
          {
            title: 'Donation Pool',
            value: stats.donationPool || 0,
            loading: isCardLoading,
            icon: Heart,
            iconColor: 'text-amber-400',
            iconBg: 'bg-amber-500/10 border-amber-500/20',
            trend: 'neutral',
            trendLabel: 'GL Donation Pool',
            trendColor: 'text-amber-400',
            accentBar: 'from-amber-500 to-amber-400',
            subLabel: stats.monthlyDonations ? `Received (${stats.currentMonthName || 'Current Month'}): Rs ${Number(stats.monthlyDonations).toLocaleString()}` : 'Dedicated Welfare Pool',
            delay: 140,
          },
          {
