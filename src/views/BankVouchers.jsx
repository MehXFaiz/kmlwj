...
export const BankVouchers = () => {
  ...
  useEffect(() => {
    fetchVouchers('BP,CP,BT');
  }, [fetchVouchers, location.key]);

  const handleSaveEdit = async (updatedData) => {
    try {
      await updateVoucher(updatedData.id, updatedData, 'BP,CP,BT');
  ...
  const handlePost = async (v) => {
    await confirm({
      ...
      action: async () => {
        await updateVoucherStatus(v.dbId, 'Posted', 'BP,CP,BT');
      }
    });
  };
  ...
  const handleCancel = async (v) => {
    await confirm({
      ...
      action: async () => {
        await updateVoucherStatus(v.dbId, 'Cancelled', 'BP,CP,BT');
      }
    });
  };
  ...
  const handleDeleteVoucher = async (v) => {
    await confirm({
      ...
      action: async () => {
        await deleteVoucher(v.dbId, 'BP,CP,BT');
      }
    });
  };
  ...
  const executeBulkDelete = async () => {
    ...
    try {
      await bulkDeleteVouchers(selectedIds, 'BP,CP,BT');
    ...
    <div className="space-y-6 pb-10">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <div className={pageActionsClass}>
          ...
          <Link to="/bank-vouchers/transfer/new"
            ...
            <ArrowUpRight className="h-4 w-4" /> Transfer Funds
          </Link>
        </div>
      </div>
