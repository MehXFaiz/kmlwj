...
import { uploadFields, handleUploadError } from "./_middlewares/upload.middleware.js";
{
  ...
  if (missingStorageVars.length === 0) {
    ...
    logger.error(
      { missingStorageVars },
      "FATAL STORAGE MISCONFIGURATION: running on Vercel with no cloud storage credentials. File uploads WILL fail until these environment variables are added in the Vercel project settings (Settings \u2192 Environment Variables) and the project is redeployed."
    );
  } else {
    logger.warn(
      { missingStorageVars },
...
const globalLimiter = rateLimit({
  windowMs: 15 * 60 * 1e3,
  // 15 minutes
  max: process.env.NODE_ENV === "production" ? 2500 : 1e4,
  // Multi-tab and multi-user desktop friendly limit (2500 req / 15 min per IP)
...
const authLimiter = rateLimit({
  windowMs: 15 * 60 * 1e3,
  // 15 minutes
  max: process.env.NODE_ENV === "production" ? 10 : 1e4,
  // Limit each IP to 10 authentication requests per window
  standardHeaders: true,
  legacyHeaders: false,
...
app.use(cors({
  origin: (origin, callback) => {
    ...
    if (allowedOrigins.length === 0 || allowedOrigins.includes(origin) || allowedOrigins.some((allowed) => origin === allowed || origin.startsWith(allowed))) {
...
app.get("/api/v1/member/verify/:id", async (req, res) => {
...
app.use((err, _req, res, _next) => {
  ...
  const message = status < 500 ? err?.message || "Request failed" : "An unexpected error occurred. Please try again.";
