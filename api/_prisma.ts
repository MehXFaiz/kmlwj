...
/**
 * Resolves the MySQL connection URL from either DATABASE_URL
...
if (!connectionUrl) {
  console.warn('[DATABASE WARNING] Neither DATABASE_URL nor DB_HOST/DB_USER/DB_NAME are set in environment variables.');
}
...
/**
 * Optional raw MySQL2 connection pool for direct MySQL operations if required.
 */
export const pool = globalForDb.mysqlPool ?? (
  process.env.DB_HOST && process.env.DB_USER && process.env.DB_NAME
    ...
    : (connectionUrl ? mysql.createPool(connectionUrl) : undefined)
);
