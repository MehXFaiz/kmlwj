...
/**
 * Single-issue "Approve & Apply" / "Reject" — the Admin-approval gate for
 * anything above LOW risk (and a manual re-trigger path for LOW risk too).
 * If the issue has no genuinely executable repair operation registered, this
 * never mutates financial data — it only records that a human acknowledged
 * the issue and will handle it through the existing manual UI.
 */
export default makeHandler(async (req: AuthenticatedRequest, res: VercelResponse) => {
  ...
  try {
    ...
    if (!op || !op.execute) {
      await prisma.aiRepairIssue.update({ where: { id: issueId }, data: { status: 'RESOLVED', resolvedAt: new Date() } });
      await prisma.aiRepairLog.create({
        data: {
          issueId,
          userId: req.user!.id,
